...
([#83](https://github.com/Daniel88dev/flexi-day/pull/83)) fails CI, and the failure sat two
layers deep, so this file records both.

## Layer 1: the `brace-expansion` override — removed

`package.json` used to pin `brace-expansion` to `^2.0.2` via `overrides`, added in
...
export. The override forced 2.x into it, and the first glob ESLint evaluated threw:

```
TypeError: (0 , brace_expansion_1.expand) is not a function
```

This is the error PR #83's CI actually shows. The override was obsolete — every published
`brace-expansion` line (1.1.12+, 2.0.2+, 5.x) carries the ReDoS fix — so it is gone, ahead of the
ESLint 10 upgrade, where it would only have masked the real compatibility work below. Natural
resolution now gives 1.1.18 under `minimatch@3` and 5.0.9 for the `minimatch@10` consumers, and
`npm audit --audit-level=high` reports zero.
...
ESLint-10-compatible version. When that ships, the upgrade PR is: bump `eslint` to `^10`,
reinstall, and confirm `npm run lint` and `npm audit --audit-level=high` both pass.
